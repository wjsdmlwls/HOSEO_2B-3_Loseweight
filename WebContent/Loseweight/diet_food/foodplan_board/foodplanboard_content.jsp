...
<%
   ...
   try{
	   ...
	   foodplanDTO article =  dbPro.getArticle(num);
...
<%
	...
	String replylistsql2="select * from foodplan_boardre where num="+article.getNum()+"";
...
function  sendProcess(f){
	f.action="foodplanboard_replyedelete.jsp";
    f.submit();      
    
}
function  sendedit(f){
	f.action="foodplanboard_replyedit.jsp";
    f.submit();      
	...
	<%=article.getFilename0()%></a><br>
		...
		<div class="reply_box">
		<form action = "foodplanboard_reply.jsp" method="post" name="replyform">
	...
	<div style="padding-bottom:60px;">
		<form method="post" action="foodplanboard_replyedit.jsp" onsubmit="return writeSave()">
			<div style="display:none">
				<input name="num" value="<%=article.getNum()%>">
				<input name="pageNum" value="<%=pageNum%>">
			</div>
	...
	<%if(id!=null){%>
						  <input type="button" value="글수정" class="all_button"
					       onclick="document.location.href='foodplanboard_updateForm.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
						   &nbsp;&nbsp;
						  <input type="button" value="글삭제" class="all_button"
 					       onclick="document.location.href='foodplanboard_deleteForm.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
						   &nbsp;&nbsp;
					      <input type="button" value="답글쓰기" class="all_button"
					       onclick="document.location.href='foodplanboard_writeForm.jsp?num=<%=num%>&ref=<%=ref%>&re_step=<%=re_step%>&re_level=<%=re_level%>'">
						   &nbsp;&nbsp;
