				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					$.each(files, function (i, file) {
						var v = String(file.name || '' ),
								...
								p = '<img class="MultiFile-preview" onchange="javascript:saveValue('+MultiFile.files.length+',this.value)"id="uploadimg'+MultiFile.files.length+'" name="uploadimg'+MultiFile.files.length+'" value="'+ n +'" style="margin-left:5px;width:100px;height:80px;'+ MultiFile.previewCss+'"/>',
								label =	$(
										(
											'<span class="MultiFile-label" title="' + t + '">'+
												'<span class="MultiFile-title"style="display:none">'+ n +'</span>'+
					...
					//$.each(files, function (i, file) {
						...
						var
							r = $('<div class="MultiFile-label"style="float:left"></div>'),
							b = $('<a class="MultiFile-remove" style="margin-left: 90;position: absolute;"href="#'+ MultiFile.wrapID + '">' + MultiFile.STRING.remove + '</a>')
								.click(function () {
