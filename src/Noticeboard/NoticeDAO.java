...
public class NoticeDAO {
	...
	public void insertArticle(NoticeDTO article) throws Exception {
		...
		try {
			...
			// 쿼리를 작성
			sql = "insert into noticeboard(writer,email,subject,passwd,reg_date,members,";
	...
	// noticeboard테이블에 저장된 전체글의 수를 얻어냄(select문)<=list.jsp에서 사용
	...
	public List<NoticeDTO> getArticles(int start, int end) throws Exception {
		...
		try {
			...
			pstmt = conn.prepareStatement("select * from noticeboard order by ref desc, re_step asc limit ?,? ");
	...
	public int updateArticle(NoticeDTO article) throws Exception {
		...
		try {
			...
			pstmt = conn.prepareStatement("select passwd from noticeboard where num = ?");
			...
			if (rs.next()) {
				...
				if (dbpasswd.equals(article.getPasswd())) {
					sql = "update noticeboard set writer=?,email=?,subject=?,passwd=?,filename0=?,filename1=?,filepath0=?,filepath1=?";
	...
	//댓글
		...
		public List<NoticeDTO> getquestionboardreList(int num) throws Exception {
			...
			try{
				...
				if(rs.next()) {
					do {
						NoticeDTO BoardDataBean = new NoticeDTO();
		...
		//수정
			public void updatereplyArticle2(NoticeDTO article)throws Exception {
				 Connection conn = null;
			        PreparedStatement pstmt = null;
			        try {
				 conn = getConnection();
