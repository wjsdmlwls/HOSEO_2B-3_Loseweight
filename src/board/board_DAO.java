...
public class board_DAO {
	...
	public void insertArticle(board_DTO article) throws Exception {
		...
		try {
			...
			// 쿼리를 작성
			sql = "insert into board(writer,email,subject,passwd,reg_date,members,";
		...
		try {
			...
			if (rs.next()) {
				...
				if (dbpasswd.equals(article.getPasswd())) {
					sql = "update board set writer=?,email=?,subject=?,passwd=?,filename0=?,filename1=?,filepath0=?,filepath1=?";
	...
	//댓글
		...
		public List<board_DTO> getquestionboardreList(int num) throws Exception {
			...
			try{
			     ...
			     conn = getConnection();			     
				 pstmt = conn.prepareStatement("select * from boardre where num=?");
				 pstmt.setInt(1,num);
				 rs = pstmt.executeQuery();
				 //값을 찾아 쿼리 업데이트
		...
		//수정
			...
			public void deletereplyArticle(board_DTO article)throws Exception {
				 ...
				 conn = getConnection();
				 	...
				 	pstmt = conn.prepareStatement("delete from boardre where glenum=?");
