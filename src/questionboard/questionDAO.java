...
public class questionDAO {
	...
	public void insertArticle(questionDTO article) throws Exception {
		...
		try {
			...
			pstmt = conn.prepareStatement("select max(num) from questionboard");
			...
			if (num != 0) {
				sql = "update questionboard set re_step=re_step+1 ";
			...
			// 쿼리를 작성
			sql = "insert into questionboard(writer,email,subject,passwd,reg_date,members,";
	...
	// questionboard테이블에 저장된 전체글의 수를 얻어냄(select문)<=list.jsp에서 사용
	...
	public List<questionDTO> getArticles(int start, int end) throws Exception {
		...
		try {
			...
			pstmt = conn.prepareStatement("select * from questionboard order by ref desc, re_step asc limit ?,? ");
			pstmt.setInt(1, start - 1);
			pstmt.setInt(2, end);
			rs = pstmt.executeQuery();

			if (rs.next()) {
				articleList = new ArrayList<questionDTO>(end);
				do {
	...
	public questionDTO getArticle(int num) throws Exception {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		questionDTO article = null;
		try {
			conn = getConnection();

			pstmt = conn.prepareStatement("select * from questionboard where num = ?");
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();

			if (rs.next()) {
				article = new questionDTO();
				article.setNum(rs.getInt("num"));
				article.setWriter(rs.getString("writer"));
				article.setEmail(rs.getString("email"));
				article.setSubject(rs.getString("subject"));
	...
	public questionDTO updateGetArticle(int num) throws Exception {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		questionDTO article = null;
	...
	public int updateArticle(questionDTO article) throws Exception {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			pstmt = conn.prepareStatement("select passwd from questionboard where num = ?");
			...
			if (rs.next()) {
				...
				if (dbpasswd.equals(article.getPasswd())) {
					sql = "update questionboard set writer=?,email=?,subject=?,passwd=?,filename0=?,filename1=?,filepath0=?,filepath1=?";
		...
		try {
			...
			if (rs.next()) {
				...
				if (dbpasswd.equals(passwd)) {
					pstmt = conn.prepareStatement("delete from questionboard where num=?");
					pstmt.setInt(1, num);
					pstmt.executeUpdate();
					x = 1; // 글삭제 성공
				} else
	...
	public List<questionDTO> getquestionboardreList(int num) throws Exception {
		...
		try{
		     ...
		     conn = getConnection();			     
			 pstmt = conn.prepareStatement("select * from questionboardre where num=?");
			 pstmt.setInt(1,num);
			 rs = pstmt.executeQuery();
			 //값을 찾아 쿼리 업데이트
			...
			if(rs.next()) {
				do {
					questionDTO BoardDataBean = new questionDTO();
	...
	//수정
		public void updatereplyArticle2(questionDTO article)throws Exception {
			 Connection conn = null;
		        PreparedStatement pstmt = null;
		        try {
			 conn = getConnection();
		...
		public void deletereplyArticle(questionDTO article)throws Exception {
			 ...
			 conn = getConnection();
			 	...
			 	pstmt = conn.prepareStatement("delete from questionboardre where glenum=?");
